...
function createOverlayManager(
  ...
  function recordError(error: string | Error, overlayId?: string): void {
    ...
    console.error(`[OverlayManager] ${message}`, { overlayId, error });
  }
  ...
  /**
   * Positions an overlay relative to its target
   */
  async function positionOverlay<T>(
    overlayInstance: OverlayInstance<T>,
    textMatch?: TextRangeMatch
  ): Promise<void> {
    if (!overlayInstance.wrapper || !overlayInstance.targetElement) {
      return;
    }
      ...
      try {
        if (registration.isTextBased) {
          ...
          const textMatches = textRangeTracker.findTextMatches(
            registration.searchFn as unknown as RegExp
          );
  ...
  function addOverlay<T>(
    ...
    const registrationId = registry.addRegistration(
      ...
      componentCreator as unknown as ElementComponentCreator<T>,
      options,
      false
    );
  ...
  function addTextOverlay<T>(
    ...
    const registrationId = registry.addRegistration(
      pattern as unknown as SearchFunction, // Cast to satisfy type - will be used as RegExp
      componentCreator as unknown as ElementComponentCreator<T>,
      options,
      true
    );
