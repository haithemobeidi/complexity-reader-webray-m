...
export interface OverlayContainerConfig {
  ...
  /** Theme configuration for overlays */
  theme?: {
    primaryColor?: string;
...
const DEFAULT_OVERLAY_STYLES = `
  ...
  * {
  ...
  /* Themed overlay styling */
  .overlay-themed .overlay-card {
    border-color: var(--overlay-primary-color, #0066cc);
    box-shadow: 0 4px 12px var(--overlay-shadow-color, rgba(0, 102, 204, 0.15));
  }

  .overlay-themed .overlay-card-header {
    background: linear-gradient(135deg, var(--overlay-primary-color, #0066cc) 0%, var(--overlay-primary-dark, #004499) 100%);
    color: white;
    border-bottom-color: var(--overlay-primary-color, #0066cc);
  }

  .overlay-themed .overlay-card-close {
    color: rgba(255, 255, 255, 0.8);
  }

  .overlay-themed .overlay-card-close:hover {
  ...
  .overlay-loading::after {
    ...
    border-top-color: var(--overlay-primary-color, #0066cc);
    border-radius: 50%;
...
export function createOverlayContainer(
  ...
  const injectStyles = (cssText: string): void => {
    ...
    try {
      ...
      console.warn('Failed to inject overlay styles:', error);
  ...
  const renderReactElement = (element: React.ReactElement, target?: HTMLElement): void => {
    ...
    if (!renderTarget) {
      console.error('No render target available for React element');
      return;
    }
    
    try {
      // Use React 18+ createRoot (required for React 19)
      const { createRoot } = ReactDOM as any;
      const root = createRoot(renderTarget);
      root.render(element);
    } catch (error) {
      console.error('Failed to render React element in overlay:', error);
  ...
  if (!container || !container.shadowRoot) {
    ...
    if (useShadowDOM) {
      try {
        ...
        shadowRoot = null; // Ensure shadowRoot is null on failure
      }
    }
  ...
  let finalStyles = DEFAULT_OVERLAY_STYLES;
  
  // Apply theme if provided
  if (theme) {
    const themeVars = `
      :root {
        --overlay-primary-color: ${theme.primaryColor || '#0066cc'};
        --overlay-primary-dark: ${theme.primaryColor ? 
          // Darken the primary color by 20%
          `color-mix(in srgb, ${theme.primaryColor} 80%, black)` : 
          '#004499'};
        --overlay-background-color: ${theme.backgroundColor || 'white'};
        --overlay-border-color: ${theme.borderColor || '#e1e5e9'};
        --overlay-shadow-color: ${theme.shadowColor || `rgba(0, 102, 204, 0.15)`};
      }
    `;
    finalStyles = themeVars + '\n' + finalStyles;
  }
  
  injectStyles(finalStyles);
