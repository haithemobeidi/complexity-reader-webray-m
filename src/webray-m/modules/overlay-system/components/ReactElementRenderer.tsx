...
class OverlayErrorBoundary extends Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('OverlayErrorBoundary caught an error:', error, errorInfo);
...
export function renderReactElement(
  ...
  const handleRetry = () => {
    ...
    retryCount++;
    console.log(`Retrying overlay render (attempt ${retryCount})`);
  ...
  const performRender = (elementToRender: React.ReactElement) => {
    ...
    try {
      ...
      if (useConcurrentFeatures) {
        ...
        // Simple root without concurrent features
        const { createRoot } = ReactDOM as any;
        if (!root) {
          root = createRoot(targetElement);
        }
      ...
      console.error('Failed to render React element:', error);
  ...
  const update = (newElement: React.ReactElement) => {
    if (isUnmounted) {
      console.warn('Cannot update unmounted overlay element');
      return;
    }
  ...
  const unmount = () => {
    ...
    try {
      ...
      console.warn('Error during overlay unmount:', error);
    }
